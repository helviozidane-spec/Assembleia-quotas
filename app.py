...
st.title("Assembleia – Cotização Mensal by HZ")
st.divider()
...
if st.session_state.is_gestor and not mes_data["fechado"]:
    ...
    if st.button("✅ Aplicar alterações"):
        ...
        def sincronizar_membros(df):
            membros_df = set(df["Nome"])
            membros_lista = set(MEMBROS)

            novos = membros_lista - membros_df
            ...
            if novos:
                for nome in novos:
                    df.loc[len(df)] = [
                        ...
                        0,
                        "Não contribuiu",
                        f"Faltam {CONTRIBUICAO_MENSAL} Kz"
                    ]
